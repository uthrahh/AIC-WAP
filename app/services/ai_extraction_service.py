...
def _extract_openai(message: str, api_key: str) -> ParsedWorklog:
    try:
        ...
        prompt = f"""
        You are extracting employee worklogs.

        Format:

        1. completed task
        2. completed task; employee1, employee2

        Pending:

        1. pending task
        2. pending task; deadline: 30-06-2026; employee1, employee2
        ...
        {{
            "completed_tasks":[
                {{
                    "task":"ERP dashboard completed",
                    "employees":["Uthrah","Dhinesh"]
                }}
            ],
            "pending_tasks":[
                {{
