...
def find_employee(
    ...
    if whatsapp_name:
        ...
        for emp in employees:
            ...
            if emp.name:
                names.extend(
                    emp.name.lower().split()
                )

            if emp.whatsapp_name:
                names.extend(
                    emp.whatsapp_name.lower().split()
                )

            if sender_words.intersection(names):
...
def process_single_message(
    ...
    *,
    ...
    if not employee:
        ...
        logger.warning(
            f"Unknown employee: {sender_name} ({sender_phone})"
        )

        return None

    if _has_update_for_day(
        db,
        employee.id,
        work_date
    ):
        ...
        logger.info(
            f"Duplicate update skipped for {employee.name} on {work_date}"
        )
    ...
    update = DailyUpdate(
        ...
        completed_tasks=tasks_to_json(
            parsed.completed_tasks
        ),

        pending_tasks=tasks_to_json(
            parsed.pending_tasks
        ),
    ...
    for completed in parsed.completed_tasks:
    ...
    for pending in parsed.pending_tasks:
